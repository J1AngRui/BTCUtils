...
import com.ruizton.main.model.JsonUtils;
...
public class BTCUtils{
	...
	//钱包IP地址
	private  String IP = "http://120.79.88.96/";
	//端口
	...
	public static void main(String args[]) throws Exception{
		...
		JSONObject objects = utils.getwalletinfo();
		System.out.println(objects);
		JSONObject object = utils.listtransactions(698858769);
		...
		for (Object object2 : allResult) {
			...
			if(map.get("category").toString().equals("receive")) {
				Double dou = (Double) map.get("amount");
				sum = sum += dou;
			}
		}
		System.out.println("当前用户已接收LMT："+sum);
	}
